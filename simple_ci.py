"""
Solves the vibrational CI problem for a QFF potential in the harmonic oscillator basis using raising
and lowering operators.

Usage:

python3 simple_ci.py input_file

input_file contains information regarding the potential energy surface, as well as the index file (see below),
and names for output files. The format is as follows:

number_of_modes		number_of_cubic_constants		number_of_quartic_constants
index_file
matrix_output_filename
ci_energy_output_filename

harmonic_frequency_1
harmonic_frequency_2
...
harmonic_frequency_n

mode_a mode_b mode_c	cubic_force_constant_1
...
mode_x mode_y mode_z	cubic_force_constant_n

mode_a mode_b mode_c mode_d		quartic_force_constant_1
...
mode_w mode_x mode_y mode_z		quartic_force_constant_n


All frequencies and force constants should be in wavenumbers (cm-1). An example file is ci_input_example.

The index file should be in your working directory and contains the number of quanta of each each mode in
each CI state to be calculated. Each row corresponds to one CI state. The file should have a number of 
columns equal to the number of modes and might look something like this for a three mode case:
...
Ensure that each row is unique. An example index file is ci_index_example.txt.

The script will print the energy of all the CI states, as well as the energy difference between each state
and the lowest energy state. These are also output in the ci energy output, specified in the input file.

A second file is also output containing the actual CI matrix, also specified in the input file.

A third file called ci_wavefunction.txt containing the ci wavefunction in terms of the ci states is created.

A few usage notes:

This script isn't super well optimized, and will likely not work for a large number of modes. I wouldn't recommend
more than 4-5 modes, although performance will vary with the number of cubic and quartic force constants
and the number of excitiations in each mode. Your mileage may vary.

Highly anharmonic modes will require a very large number (>10) of excitations to describe accurately.

Large cubic or negative quartic terms can cause a "turning over" of the potential, where the potential minima
is not at zero. This can result in unphysically low energies (possibly negative). Reducing the number of excitations,
particularly of low frequency modes, can solve the problem, but, fundamentally, this is an issue with the QFF
potential, and a different description of the potential energy surface with correct limiting behavior is needed.
